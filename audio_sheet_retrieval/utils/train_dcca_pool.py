...
def fit(layers, data, objectives,
    ...
    try:
        ...
        prev_rank_tr, prev_rank_va = 1e7, 1e7
        ...
        for epoch in train(iter_funcs, data, train_batch_iter, valid_batch_iter, fit_cca):
            ...
            map_tr, map_va = epoch['map_tr'], epoch['map_va']
            med_rank_tr, med_rank_va = epoch['med_rank_tr'], epoch['med_rank_va']
            evals_tr = epoch['evals_tr']

            # prepare early stopping
            # improvement = va_dist < prev_va_dist
            # improvement = rank_va < prev_rank_va
            # improvement = med_rank_va < prev_med_rank_va
            improvement = map_va >= prev_map_va
            if improvement:
                last_improvement = 0
            ...
            txt_tr_map = 'map_tr %.2f ' % (100 * map_tr)
            if map_tr > prev_map_tr:
                txt_tr_map = col.print_colored(txt_tr_map, BColors.OKGREEN)
                prev_map_tr = map_tr

            txt_va_map = 'map_va %.2f ' % (100 * map_va)
            if map_va > prev_map_va:
            ...
            maps_tr.append(map_tr)
            maps_va.append(map_va)
            ...
            if last_improvement > patience:
                ...
                best = (best_epoch, prev_va_loss, prev_va_dist, 100 * prev_map_va)
