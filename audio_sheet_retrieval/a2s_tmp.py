...
from multi_modality_hashing.config.settings import EXP_ROOT
from multi_modality_hashing.config.settings import DATA_ROOT_REAL_MUSIC as ROOT_DIR
from utils.real_music_data import prepare_pool, SPEC_CONTEXT, SPEC_BINS, te_pieces, NO_AUGMENT
from retrieval_wrapper import RetrievalWrapper

from score_alignment.utils.midi_tools import get_online_spec_processor
...
class AudioSheetServer(object):
    ...
    def __init__(self):
        ...
        self.online_spec_proc = get_online_spec_processor(gain=100)
        ...
        try:
            ...
            for i_frame, Frame in enumerate(spectrogram_generator):
                ...
                if gui:
                    ...
                    plt.imshow(running_spec, cmap=cmaps['viridis'], origin="lower")
                    plt.grid('off')
                    plt.axis('off')
    ...
    def initialize_sheet_db(self, keep_snippets=True):
        ...
        from utils.real_music_data import te_pieces, va_pieces
        pieces = te_pieces + va_pieces
        for pice_idx, piece in enumerate(pieces):
            print(" (%03d / %03d) %s" % (pice_idx + 1, len(pieces), piece))
            piece_pool = prepare_pool([piece], data_augmentation=NO_AUGMENT, shuffle=False, one_spec=True, versions=[None])
            ...
            for j in xrange(piece_pool.shape[0]):
                ...
                # re-clip image
                r0 = image.shape[2] // 2 - self.snippet_shape[0] // 2
                r1 = r0 + self.snippet_shape[0]
                image = image[:, :, r0:r1, :]
    ...
    def _mic_spec_gen(self):
        """ frame from mic generator """
        frames = online_frame_generator()
        for frame in frames:
            Frame = self.online_spec_proc(frame).T
            yield Frame
...
if __name__ == '__main__':
    ...
    if args.init_db:
        a2s_srv.initialize_sheet_db(keep_snippets=False)
    ...
    if args.full_eval:
        ...
        for tp in te_pieces:
            ...
            if args.real_audio:
                ...
                audio_file = os.path.join(ROOT_DIR, "%s/audio/%s_bpm_120_Steinway.flac")
                audio_file %= (tp, tp)
            ...
            txt = ("Query Audio: %s" % os.path.basename(audio_file)).ljust(70)
            print(txt, end='')
            ...
            print(" " + col.print_colored("rank: %d (%.2f)" % (ranks[-1], ratio), color))
        ...
        test_piece = "Mozart_Piano_Sonata_K545_1"
        # test_piece = "Chopin_Prelude_Op28_No4"
        # test_piece = "Chopin_Nocturne_Op9_No1"
        # test_piece = "Beethoven_Piano_Sonata_1_1"
        if args.real_audio:
            # audio_file = "/home/matthias/cp/data/sheet_localization/real_music/0_real_audio/%s.flac" % test_piece
            audio_file = "/home/matthias/cp/data/sheet_localization/real_music/0_retrieval_samples/%s.flac" % test_piece
        else:
            audio_file = "/home/matthias/cp/data/sheet_localization/real_music/%s/audio/%s_bpm_120_Steinway.flac"
            audio_file %= (test_piece, test_piece)
        ...
        # start service
        a2s_srv.run(spec, top_k=7, n_candidates=args.n_candidates, running_frames=args.running_frames, target_piece=test_piece)
